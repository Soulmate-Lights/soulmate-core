...
#ifndef BUILDER_LIBRARIES_SOULMATE_SOULMATEMAIN_CPP_
#define BUILDER_LIBRARIES_SOULMATE_SOULMATEMAIN_CPP_

// #define SOULMATE_VERSION "8.0.4"

#ifndef FIRMWARE_NAME
  #define FIRMWARE_NAME "My firmware"
#endif
...
String SoulmateLibrary::status(bool showLANIP) {
...
void SoulmateLibrary::stop() {
  stopped = true;
}

bool SoulmateLibrary::isStopped() {
  return stopped;
}

// Light up a percentage of the panel to represent update status
void SoulmateLibrary::lightPercentage(float percentage) {
  fill_solid(this->leds, N_LEDS, CRGB::Black);
  if (percentage < 0.9) {
...
void SoulmateLibrary::nextRoutine() {
  if (currentRoutine < 0)
    return;
...
void SoulmateLibrary::adjustBrightness() {
  if (on) {
...
void SoulmateLibrary::playCurrentRoutine() {
  if (currentRoutine == -2) {
...
void SoulmateLibrary::factoryReset() {
  deleteAllFiles();
  disconnectWiFi();
  ESP.restart();
}

void SoulmateLibrary::reverseLeds() {
  uint16_t left = 0;
...
void SoulmateLibrary::fastLedShow() {
...
void SoulmateLibrary::showPixels() {
  if (isStopped())
    return;
...
void SoulmateLibrary::adjustFromButton(){
#ifdef SOULMATE_BUTTON_PIN
...
void SoulmateLibrary::addRoutine(String routineName, void (*routine)()) {
  routines[routineCount] = routine;
  routineNames[routineCount] = routineName;
  routineCount++;
}

void SoulmateLibrary::setPixel(int index, CRGB color) {
  leds[index] = color;
}

void SoulmateLibrary::chooseRoutine(int i) {
  if (i == currentRoutine)
    return;
...
void SoulmateLibrary::setBrightness(int b) {
  if (startingFrames >= b)
    startingFrames = 255;
  if (b > 0 && !on)
    on = true;
...
void SoulmateLibrary::turnOff() {
  on = false;
}

void SoulmateLibrary::turnOn() {
  on = true;
}

void SoulmateLibrary::toggleOnOff() {
  on = !on;
}

float SoulmateLibrary::CurrentBrightnessAsFloat() {
  return static_cast<float>(FastLED.getBrightness()) / 255.0;
}

void SoulmateLibrary::setName(String n) {
  if (n.length() == 0)
    return;
...
void SoulmateLibrary::onJSON(std::function<void(const JsonObject &)> callback) {
  _jsonCallback = callback;
}

void SoulmateLibrary::consumeJson(const JsonObject &root) {
  ...
  if (root.containsKey("time")) {
    ...
    unsigned long newStartedSeconds = receivedSeconds - currentSeconds;
    Circadian::startTrackingTime(newStartedSeconds);
  }
  ...
  if (root.containsKey("wakeTime") && root.containsKey("sleepTime")) {
    float newWakeTime = static_cast<float>(root["wakeTime"]);
    float newSleepTime = static_cast<float>(root["sleepTime"]);
    Circadian::setSleepTime(newSleepTime);
    Circadian::setWakeTime(newWakeTime);
  }
...
#endif // BUILDER_LIBRARIES_SOULMATE_SOULMATEMAIN_CPP_
